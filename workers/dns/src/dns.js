...
// validate subdomain segment only (no dots allowed prevents zone escape)
function isValidSubdomain(sub) {
	// single label: a-z0-9 + hyphens; no dots; 2-63 chars
...
function isValidRecordName(name) {
	...
	// allow single label only e.g. "_dmarc" but not [cool.aitji].id.thatako.net
	// btw: if you want sub sub domain ; contact me
	return /^[a-z0-9_][a-z0-9_\-]{0,61}[a-z0-9_]?$/i.test(name) && !name.includes('.')
}
...
function checkPayload(payload) {
	...
	if (!payload.domain || typeof payload.domain !== 'string') {
		...
		return errors // bail early ; rest of checks need domain
	}

	// extract subdomain; reject anything with extra dots (zone bypass)
	const domainParts = payload.domain.split('.')
	// expected: [sub, id, thatako, net] 4 parts exactly
	if (domainParts.length !== 4 || domainParts.slice(1).join('.') !== 'id.thatako.net') errors.push('domain must be exactly [name].id.thatako.net')

	const sub = domainParts[0]
	if (!isValidSubdomain(sub)) errors.push('subdomain label invalid (a-z0-9 and - only, 2-63 chars)')
	if (DISALLOWED.has(sub)) errors.push('subdomain is reserved')

	// guard path traversal just in case
	...
	for (const o of (payload.owner || [])) {
		if (typeof o.github !== 'string' || typeof o['github-id'] !== 'number')
			errors.push('each owner must have github (string) & github-id (number)')
	}

	if (!payload.records || typeof payload.records !== 'object') {
		errors.push('records object required')
		return errors
	}
	...
	for (const [type, val] of Object.entries(payload.records)) {
		...
		for (const v of values) {
			// each record must be {name, value}
			if (typeof v !== 'object' || !v) { errors.push(`${type}: record must be object {name, value}`); continue }

			const name = typeof v.name === 'string' ? v.name : ''
			const value = typeof v.value === 'string' ? v.value : ''

			if (!isValidRecordName(name)) errors.push(`${type}: invalid record name "${name}" (@ or single label only no dots)`)
			if (DANGEROUS.test(value) || DANGEROUS.test(name)) errors.push(`${type}: dangerous content in record`)

			// extra: value must not be a thatako.net subdomain that isn't id.thatako.net
			// blocks users pointing CNAMEs back to internal zones
			if ((type === 'CNAME' || type === 'MX') && /thatako\.net$/i.test(value)) {
				if (!/\.id\.thatako\.net\.?$/.test(value)) {
					errors.push(`${type}: target "${value}" points to internal thatako.net zone`)
				}
			}
		}
	}
...
export default {
	async fetch(request, env) {
		...
		if (request.method === 'GET') {
			...
			for (const result of settled) {
				...
				try {
					...
					if (type === 'full') domains.push(data)
					else {
		...
		if (request.method === 'POST') {
			...
			// safe file path: subdomain is already validated as single-label no-dot
			...
			if (existing.ok) {
				...
				} catch { /* allow overwrite if malformed */ }
			}
		...
		if (request.method === 'DELETE') {
			...
			if (!domain || typeof domain !== 'string') return json({ error: 'domain required' }, 400)

			// same strict check as POST
			const parts = domain.split('.')
			if (parts.length !== 4 || parts.slice(1).join('.') !== 'id.thatako.net')
				return json({ error: 'invalid domain' }, 400)

			const subdomain = parts[0]
			if (!isValidSubdomain(subdomain)) return json({ error: 'invalid domain' }, 400)
